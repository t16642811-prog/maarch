...
export class ProcessComponent implements OnInit, OnDestroy {
    ...
    actionsList: any[] = [];
    allResources: any[] = [];
    allowActionsInDetail: boolean = false;
    ...
    isAnamTreated(): boolean {
        if (this.currentResourceInformations?.anamWorkflowStep === 'validated') {
            return true;
        }
    ...
    private loadResponseAttachment() {
        if (this.functions.empty(this.currentResourceInformations.resId)) {
            this.showResponseSplit = false;
            this.responseAttachment = null;
            return;
        }
        this.http.get(`../rest/resources/${this.currentResourceInformations.resId}/attachments`).pipe(
            tap((data: any) => {
                const attachments = data.attachments || [];
                this.responseAttachment = this.pickResponseAttachment(attachments);
                this.showResponseSplit = !this.functions.empty(this.responseAttachment);
            }),
            catchError((err: any) => {
                this.notify.handleSoftErrors(err);
                this.showResponseSplit = false;
    ...
    private pickResponseAttachment(attachments: any[]): any {
        const responseTypeIds = ['response_project', 'signed_response'];
        const responseAttachments = attachments.filter((attachment: any) => {
            const typeLabel = (attachment.typeLabel || '').toLowerCase();
            return responseTypeIds.includes(attachment.type) || typeLabel.includes('reponse') || typeLabel.includes('réponse');
        });
        if (responseAttachments.length === 0) {
            return null;
        }
        responseAttachments.sort((a: any, b: any) => (a.resId || 0) - (b.resId || 0));
        return responseAttachments[responseAttachments.length - 1];
    }

    openSplitPreview(target: 'main' | 'response') {
        if (target === 'main') {
            this.http.get(`../rest/resources/${this.currentResourceInformations.resId}/content?mode=base64`).pipe(
                tap((data: any) => {
                    this.dialog.open(DocumentViewerModalComponent, {
                        autoFocus: false,
                        panelClass: ['maarch-full-height-modal', 'maarch-doc-modal'],
                        data: {
                            title: `Courrier recu - ${data.filename}`,
        ...
        if (this.functions.empty(this.responseAttachment)) {
            return;
        }

        this.http.get(`../rest/attachments/${this.responseAttachment.resId}/content?mode=base64`).pipe(
            tap((data: any) => {
                this.dialog.open(DocumentViewerModalComponent, {
                    autoFocus: false,
                    panelClass: ['maarch-full-height-modal', 'maarch-doc-modal'],
                    data: {
                        title: `Reponse - ${data.filename}`,
                        base64: data.encodedDocument,
                        filename: data.filename
                    }
                });
