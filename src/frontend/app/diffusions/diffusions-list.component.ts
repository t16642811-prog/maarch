...
export class DiffusionsListComponent implements OnInit {
    ...
    anamRoles: { id: string; label: string }[] = [
        { id: 'A executer', label: 'A exécuter' },
        { id: 'Pour suivi', label: 'Pour suivi' },
        ...
        try {
            ...
            if (listTemplates.length > 0) {
                listTemplates[0].forEach((element: any) => {
                    if (this.diffList[element.item_mode]) {
            ...
            if (destCurrentUser && this.headerService.user.entities[0].id === entityId && this.diffList['dest']) {
    ...
    getAnamRole(diffusion: any): string {
        const allowed = this.anamRoles.map(role => role.id);
        return allowed.includes(diffusion.process_comment) ? diffusion.process_comment : '';
    }

    setAnamRole(diffusion: any, value: string) {
        diffusion.process_comment = value;
        if (this.diffFormControl !== undefined) {
            this.setFormValues();
        }
    }
    ...
    addEntityDest(element: any) {
        if (!this.diffList || !this.diffList['dest']) {
            return;
        }
        const newElemListModel: any = {
            listinstance_id: null,
        ...
        if (!this.isItemInThisRole(newElemListModel, 'dest')) {
            this.diffList['dest'].items.unshift(newElemListModel);
            if (this.diffFormControl !== undefined) {
                this.setFormValues();
            }
        }
