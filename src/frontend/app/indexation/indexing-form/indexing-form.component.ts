...
export class IndexingFormComponent implements OnInit {
    ...
    anamFormEnabled: boolean = false;
    anamFieldIdentifiers: string[] = [
        'indexingCustomField_6',
        'indexingCustomField_7',
        'indexingCustomField_8',
        'indexingCustomField_9',
        'indexingCustomField_10',
        'indexingCustomField_11',
        'indexingCustomField_12',
        'indexingCustomField_13',
        'indexingCustomField_14',
        'indexingCustomField_15',
    ...
    anamFieldIds: any = {
        ...
        bog: 'indexingCustomField_7',
        mem: 'indexingCustomField_8',
        sg: 'indexingCustomField_9',
        cc: 'indexingCustomField_10',
        dgm: 'indexingCustomField_11',
        destExec: 'indexingCustomField_12',
        destFollow: 'indexingCustomField_13',
        destInfo: 'indexingCustomField_14',
        instructionsPcd: 'indexingCustomField_15',
    ...
    async initElemForm(saveResourceState: boolean = true) {
        ...
        this.anamFormEnabled = this.hasAnamFormFields();
        this.loading = false;
    }
                ...
                tap(async (data: any) => {
                    ...
                    await Promise.all(this.fieldCategories.map(async (element: any) => {
                        ...
                        await Promise.all(this['indexingModels_' + element].map(async (elem: any) => {
                            ...
                            if (!this.canEdit) {
                                const anamEditableInProcess = this.mode === 'process' && this.isAnamField(elem) &&
                                    [
    ...
    isAnamField(field: any): boolean {
        return this.anamFieldIdentifiers.includes(field.identifier);
    }

    getFieldByIdentifier(identifier: string): any {
        const categories = this.fieldCategories.map((category) => this['indexingModels_' + category]);
        for (const categoryFields of categories) {
            const match = categoryFields.find((field: any) => field.identifier === identifier);
            if (match) {
                return match;
            }
        }
    ...
    isCheckboxSelected(fieldId: string, valueId: any): boolean {
        const currentValue = this.arrFormControl[fieldId]?.value ?? [];
        const normalized = String(valueId);
        return Array.isArray(currentValue) && currentValue.map((val: any) => String(val)).includes(normalized);
    }

    toggleCheckbox(fieldId: string, valueId: any, checked: boolean) {
        const currentValue = this.arrFormControl[fieldId]?.value ?? [];
        const nextValue = Array.isArray(currentValue) ? [...currentValue] : [];
        const normalized = String(valueId);
        const normalizedCurrent = nextValue.map((val: any) => String(val));
        if (checked) {
            if (!normalizedCurrent.includes(normalized)) {
                nextValue.push(normalized);
            }
        } else {
            const filtered = nextValue.filter((val: any) => String(val) !== normalized);
            nextValue.length = 0;
    ...
    getAnamDestRows(): any[] {
        return [
            {
                id: this.anamFieldIds.destExec,
                label: this.getFieldByIdentifier(this.anamFieldIds.destExec)?.label || 'A executer'
            },
            {
                id: this.anamFieldIds.destFollow,
                label: this.getFieldByIdentifier(this.anamFieldIds.destFollow)?.label || 'Pour suivi'
            },
            {
    ...
    getAnamDestRowLabel(label: string): string {
        ...
        return label.replace(/^Destinataires\\s*-\\s*/i, '');
    }
    ...
    private hasAnamDestinationSelection(): boolean {
        ...
        return (Array.isArray(destExec) && destExec.length > 0) ||
            (Array.isArray(destFollow) && destFollow.length > 0) ||
            (Array.isArray(destInfo) && destInfo.length > 0);
    }

    getAnamProcessErrors(): string[] {
    ...
    getSelectedReceptionMode(): string {
        return this.arrFormControl[this.anamFieldIds.mode]?.value ?? '';
    }
    ...
    downloadAnamPdf() {
        if (this.resId === null) {
            this.notify.handleSoftErrors({ error: { errors: 'Document is not saved yet' } });
            return;
        }
        const downloadLink = document.createElement('a');
        this.http.get(`../rest/resources/${this.resId}/anamFormPdf`, { responseType: 'blob' }).pipe(
            tap((data: any) => {
                const blobUrl = URL.createObjectURL(data);
                downloadLink.href = blobUrl;
                downloadLink.setAttribute('download', `fiche_courrier_arrivee_${this.resId}.pdf`);
                ...
                if (err?.error instanceof Blob) {
                    ...
                    reader.onload = () => {
                        try {
                            ...
                            this.notify.handleSoftErrors({ error: { errors: payload.errors || payload.error || reader.result } });
                        } catch (e) {
                            this.notify.handleSoftErrors({ error: { errors: reader.result } });
                        }
                    };
    ...
    downloadAnamPdfDebug() {
        if (this.resId === null) {
            this.notify.handleSoftErrors({ error: { errors: 'Document is not saved yet' } });
            return;
        }
        const downloadLink = document.createElement('a');
        this.http.get(`../rest/resources/${this.resId}/anamFormPdf?debug=1`, { responseType: 'blob' }).pipe(
            tap((data: any) => {
                const blobUrl = URL.createObjectURL(data);
                downloadLink.href = blobUrl;
                downloadLink.setAttribute('download', `fiche_courrier_arrivee_${this.resId}_debug.pdf`);
                document.body.appendChild(downloadLink);
                downloadLink.click();
                URL.revokeObjectURL(blobUrl);
                downloadLink.remove();
