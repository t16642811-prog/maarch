...
class CoreController
{
    ...
    public static function getMimeTypeAndFileSize(array $args): array
    {
        ...
        // Compute mime type using native function if available, else fall back to finfo (or octet-stream).
        $mimeType = null;
        ...
        if (empty($mimeType)) {
            if (class_exists('\finfo')) {
                ...
                $buffer = stream_get_contents($resource, 2048, 0);
                $mimeType = $finfo->buffer($buffer) ?: null;
            }
        }
        fclose($resource);
