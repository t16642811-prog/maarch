...
class TextFormatModel
{
    public static function normalize(array $aArgs)
    {
        ...
        $string = self::toIso88591($aArgs['string']);
        $string = strtr($string, self::toIso88591($a), $b);
        $string = strtolower($string);

        return self::toUtf8($string);
    }
    ...
    public static function formatFilename(array $args)
    {
        ...
        return preg_replace('@[\\/:*?"\'<>|,]@', '_', $args['filename']);
    }

    private static function toIso88591(string $value): string
    {
