...
abstract class BasketModelAbstract
{
     ...
     * @throws Exception
     */
    public static function getByBasketId(array $aArgs): array
    {
     ...
     * @throws Exception
     */
    public static function getBasketsByLogin(array $aArgs): array
    {
        ...
        if (!empty($groupIds)) {
            $where = [
                'groupbasket.group_id in (?)',
                'groupbasket.basket_id = baskets.basket_id',
                'groupbasket.group_id = usergroups.group_id'
            ];
            $data = [$groupIds];
            ...
            $aBaskets = DatabaseModel::select([
                'select'   => [
                    'usergroups.id as groupSerialId',
                    'groupbasket.basket_id',
                    'baskets.id',
            ...
            foreach ($aBaskets as $key => $value) {
                ...
                $aBaskets[$key]['userToDisplay'] = (empty($redirectedBasket[0])
                    ? null
                    : UserModel::getLabelledUserById(['id' => $redirectedBasket[0]['actual_user_id']]));
                $aBaskets[$key]['enabled'] = true;
                $aBaskets[$key]['allowed'] = false;
                foreach ($userPrefs as $userPref) {
                    if (
                        $userPref['group_serial_id'] == $value['groupserialid'] &&
                        $userPref['basket_id'] == $value['basket_id']
                    ) {
     ...
     * @throws Exception
     */
    public static function getRegroupedBasketsByUserId(array $aArgs): array
    {
     ...
     * @throws Exception
     */
    public static function getAvailableBasketsByGroupUser(array $aArgs): array
    {
     ...
     * @throws Exception
     */
    public static function getDefaultActionIdByBasketId(array $aArgs): mixed
    {
     ...
     * @throws Exception
     */
    public static function getResourceNumberByClause(array $args): int
    {
     ...
     * @throws Exception
     */
    public static function getWithPreferences(array $args): array
    {
        ValidatorModel::arrayType($args, ['select', 'where', 'data', 'orderBy']);

        $where = [
            '(baskets.basket_id = users_baskets_preferences.basket_id AND' .
            ' users_baskets_preferences.group_serial_id = usergroups.id)'
        ];
        if (!empty($args['where'])) {
