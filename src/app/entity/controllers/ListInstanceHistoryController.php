...
class ListInstanceHistoryController
{
     ...
     * @throws Exception
     */
    public function getDiffusionListByResId(Request $request, Response $response, array $args): Response
    {
        if (
            ...
            foreach ($listInstancesDetails as $listInstancesDetail) {
                ...
                if ($listInstancesDetail['item_type'] == 'entity_id') {
                    $entity = EntityModel::getById(
                        ['id' => $listInstancesDetail['item_id'], 'select' => ['entity_label', 'entity_id']]
                    );
                    ...
                    $listInstancesDetail['itemSerialId'] = $listInstancesDetail['item_id'];
                    $listInstancesDetail['itemLabel'] = UserModel::getLabelledUserById(
                        ['id' => $listInstancesDetail['item_id']]
                    );
                    $listInstancesDetail['itemSubLabel'] = UserModel::getPrimaryEntityById(
            ...
            if (!empty($listInstancesDetails)) {
                if ($limKey == 0) {
                    ...
                    $formattedHistory[] = [
                        ...
                        'user'         => UserModel::getLabelledUserById(
                            ['id' => $listInstancesModification[$limKey - 1]['user_id']]
                        ),
     ...
     * @throws Exception
     */
    public function getCircuitByResId(Request $request, Response $response, array $args): Response
    {
        if (
            !Validator::intVal()->validate($args['resId']) ||
            !ResController::hasRightByResId(['resId' => [$args['resId']], 'userId' => $GLOBALS['id']])
        ...
        foreach ($listInstancesModification as $value) {
            ...
            foreach ($listInstancesDetails as $key => $listInstancesDetail) {
                ...
                $listInstancesDetails[$key]['itemLabel'] = UserModel::getLabelledUserById(
                    ['id' => $listInstancesDetail['item_id']]
                );
