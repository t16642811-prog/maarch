...
class AlfrescoController
{
    ...
    public function updateConfiguration(Request $request, Response $response): Response
    {
        ...
        if (empty($configuration)) {
            ...
            ConfigurationModel::update(
                ['set' => ['value' => $value], 'where' => ['privilege = ?'], 'data' => ['admin_alfresco']]
            );
        }
    ...
    public function updateAccount(Request $request, Response $response, array $args): Response
    {
        ...
        $account = [
            ...
            'label'    => $body['label'],
            'login'    => $body['login'],
            'password' => empty($body['password']) ? $alfresco['alfresco']['password'] : PasswordController::encrypt(
    ...
    public function checkAccount(Request $request, Response $response): Response
    {
        ...
        if (!Validator::stringType()->notEmpty()->validate($body['login'])) {
            ...
            !Validator::stringType()->notEmpty()->validate($body['accountId'])
        ) {
            return $response->withStatus(400)->withJson(['errors' => 'Body password is empty or not a string']);
        }

        if (empty($body['password'])) {
            $account = EntityModel::get(
                [
                    'select' => ['external_id'],
                    'where'  => ["external_id->'alfresco'->>'id' = ?"],
                    'data'   => [$body['accountId']],
        ...
        if ($curlResponse['code'] != 200) {
            if (!empty($curlResponse['response']['error']['briefSummary'])) {
                return $response->withStatus(400)->withJson(
                    ['errors' => $curlResponse['response']['error']['briefSummary']]
                );
     ...
     * @throws Exception
     */
    public function getRootFolders(Request $request, Response $response): Response
    {
        $configuration = ConfigurationModel::getByPrivilege(['privilege' => 'admin_alfresco']);
        if (empty($configuration)) {
            return $response->withStatus(400)->withJson(['errors' => 'Alfresco configuration is not enabled']);
        ...
        $entityInformations['alfresco']['password'] = PasswordController::decrypt(
            ['encryptedData' => $entityInformations['alfresco']['password']]
        );
     ...
     * @throws Exception
     */
    public function getChildrenFoldersById(Request $request, Response $response, array $args): Response
    {
        $configuration = ConfigurationModel::getByPrivilege(['privilege' => 'admin_alfresco']);
        if (empty($configuration)) {
            return $response->withStatus(400)->withJson(['errors' => 'Alfresco configuration is not enabled']);
     ...
     * @throws Exception
     */
    public function getFolders(Request $request, Response $response): Response
    {
        ...
        $body = [
            'query'  => [
                ...
                and IN_TREE('{$entityInformations['alfresco']['nodeId']}')",
                'language' => 'cmis',
            ],
            'fields' => ['id', 'name']
        ];
     ...
     * @throws Exception
     */
    public static function sendResource(array $args): array
    {
        ...
        if ($curlResponse['code'] != 201) {
            return [
                'errors' => "Create folder {$document['alt_identifier']} failed : " .
                    json_encode($curlResponse['response'])
            ];
        }
        ...
        $alfrescoCharToTrim = '. ';
        $document['subject'] = str_replace(
            $alfrescoCharRefused,
            ' ',
            $document['subject']
        ); // replace refused characters with a blank space
        $document['subject'] = preg_replace(
            '/\s+/u',
            ' ',
            $document['subject']
        ...
        if (!empty($alfrescoParameters['mapping']['document'])) {
            ...
            foreach ($alfrescoParameters['mapping']['document'] as $key => $alfrescoParameter) {
                if ($alfrescoParameter == 'alfrescoLogin') {
                    ...
                    if (!empty($document['initiator'])) {
                        $initiator = EntityModel::getByEntityId(
                            ['entityId' => $document['initiator'], 'select' => ['entity_label']]
                        );
                ...
                } elseif (str_contains($alfrescoParameter, 'senderCompany_')) {
                    $contactNb = explode('_', $alfrescoParameter)[1];
                    $properties[$key] = $rawContacts[$contactNb]['company'] ?? '';
                } elseif (str_contains($alfrescoParameter, 'senderCivility_')) {
                    $contactNb = explode('_', $alfrescoParameter)[1];
                    $civility = null;
                    if (!empty($rawContacts[$contactNb]['civility'])) {
                        $civility = ContactCivilityController::getLabelById(
                            ['id' => $rawContacts[$contactNb]['civility']]
                        );
                    }
                    $properties[$key] = $civility ?? '';
                } elseif (str_contains($alfrescoParameter, 'senderFirstname_')) {
                    $contactNb = explode('_', $alfrescoParameter)[1];
                    $properties[$key] = $rawContacts[$contactNb]['firstname'] ?? '';
                } elseif (str_contains($alfrescoParameter, 'senderLastname_')) {
                    $contactNb = explode('_', $alfrescoParameter)[1];
                    $properties[$key] = $rawContacts[$contactNb]['lastname'] ?? '';
                } elseif (str_contains($alfrescoParameter, 'senderFunction_')) {
                    $contactNb = explode('_', $alfrescoParameter)[1];
                    $properties[$key] = $rawContacts[$contactNb]['function'] ?? '';
                } elseif (str_contains($alfrescoParameter, 'senderAddress_')) {
                    $contactNb = explode('_', $alfrescoParameter)[1];
                    if (!empty($rawContacts[$contactNb])) {
                        $contactToDisplay = ContactController::getFormattedContactWithAddress(
                            ['contact' => $rawContacts[$contactNb]]
                        );
                    }
                    ...
                    if (!empty($document['type_id'])) {
                        $doctype = DoctypeModel::getById(
                            ['select' => ['doctypes_second_level_id'], 'id' => $document['type_id']]
                        );
                ...
                } elseif (str_contains($alfrescoParameter, 'customField_')) {
                    $customId = explode('_', $alfrescoParameter)[1];
                    $customValue = json_decode($document['custom_fields'], true);
                    $properties[$key] = (!empty($customValue[$customId]) &&
                        is_string($customValue[$customId])) ? $customValue[$customId] : '';
        ...
        foreach ($attachments as $attachment) {
            ...
            if ($firstAttachment) {
                ...
                if ($curlResponse['code'] != 201) {
                    return [
                        'errors' => "Create folder 'Pièces jointes' failed : " . json_encode($curlResponse['response'])
                    ];
                }
            ...
            if ($curlResponse['code'] != 201) {
                return [
                    'errors' => "Send attachment {$attachment['res_id']} failed : " .
                        json_encode($curlResponse['response'])
                ];
            }
                ...
                foreach ($alfrescoParameters['mapping']['attachment'] as $key => $alfrescoParameter) {
                    if ($alfrescoParameter == 'typeLabel') {
                        $attachmentType = AttachmentTypeModel::getByTypeId(
                            ['select' => ['label'], 'typeId' => $attachment['attachment_type']]
                        );
            ...
            if ($curlResponse['code'] != 200) {
                return [
                    'errors' => "Update attachment {$attachment['res_id']} failed : " .
                        json_encode($curlResponse['response'])
                ];
            }
        ...
        if (!empty($alfrescoParameters['mapping']['folderModification'])) {
            ...
            if ($curlResponse['code'] != 200) {
                return [
                    'errors' => "Update alfresco folder {$resourceFolderId} failed : " .
                        json_encode($curlResponse['response'])
                ];
            }
        }
