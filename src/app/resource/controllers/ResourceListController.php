...
class ResourceListController
{
    ...
    public function get(Request $request, Response $response, array $aArgs): Response
    {
        ...
        if (!empty($resIds)) {
            ...
            $selectData = ResourceListController::getSelectData([
            ...
            foreach ($formattedResources as &$formattedResource) {
                $resId = (int)($formattedResource['resId'] ?? 0);
                $step = (string)($anamSteps[$resId] ?? '');
                $formattedResource['anamWorkflowStep'] = $step;
                $formattedResource['anamTreated'] = $step === 'validated';
            }
            unset($formattedResource);
    ...
    public static function getSelectData(array $args): array
    {
        $select = [
            ...
            'priorities.label AS "priorities.label"',
        ...
        if (!empty($args['userId']) && !empty($args['basketId'])) {
            $userId = (int)$args['userId'];
            $basketId = addslashes((string)$args['basketId']);
            $select[] = 'CASE WHEN res_mark_as_read.user_id IS NULL THEN 0 ELSE 1 END AS "is_read"';
            $tableFunction[] = 'res_mark_as_read';
            $leftJoinFunction[] = "res_mark_as_read.res_id = res_letterbox.res_id AND res_mark_as_read.user_id = {$userId} AND res_mark_as_read.basket_id = '{$basketId}'";
        }
    ...
    public static function getResourcesListQueryData(array $args): array
    {
        ...
        if (!empty($args['data']['order']) && str_contains($args['data']['order'], 'alt_identifier')) {
            $order = 'order_alphanum(alt_identifier) ' . explode(' ', $args['data']['order'])[1];
        }
        if (!empty($args['data']['order']) && str_contains($args['data']['order'], 'dest_user')) {
        ...
        if (!empty($args['data']['order']) && str_contains($args['data']['order'], 'priority')) {
    ...
    public function getActions(Request $request, Response $response, array $args): Response
    {
        ...
        if (!empty($actions)) {
            $actions = ActionModel::get(
                [
                    'select'  => ['id', 'label_action', 'component'],
                    'where'   => ['id in (?)'],
                    'data'    => [$actions],
    ...
    private static function getAnamWorkflowStepsByResIds(array $resIds): array
    {
        if (empty($resIds)) {
            return [];
        }
        ...
        $steps = [];
        foreach ($rows as $row) {
            $customFields = !empty($row['custom_fields']) ? json_decode($row['custom_fields'], true) : [];
            $workflow = is_array($customFields) ? ($customFields['_anamWorkflow'] ?? null) : null;
            $steps[(int)$row['res_id']] = is_array($workflow) ? (string)($workflow['step'] ?? '') : '';
        }
    ...
    public static function getFormattedResources(array $args): array
    {
        ...
        foreach ($resources as $key => $resource) {
            ...
            $formattedResources[$key]['priorityLabel'] = $resource['priorities.label'] ?? null;
